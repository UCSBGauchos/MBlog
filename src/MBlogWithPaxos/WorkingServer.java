	...
	public void run() {
		...
		try {
	    	...
	    	if(str!=null){
				...
				String ackIP = commonFunc.getAckIP(ackContent);
				if(!localRep.paxosHistory[paxosInstance].ackIPs.contains(ackIP)){
...
//	    					here just for testing, in the real environment, it should be unique******************
	    					if(accValue!=null){
							   ...
							   ||accNumber==maxAccNum&&accPID>maxAccPID){
				...
				String acceptIP = commonFunc.getAcceptIP(acceptContent);
	    			if(!localRep.paxosHistory[paxosInstance].acceptIPs.contains(acceptIP)){
	    				localRep.paxosHistory[paxosInstance].acceptIPs.add(acceptIP);
	    			}
	    			if(sendBalNum>localRep.paxosHistory[paxosInstance].promiseBal.balNumber
				   ||sendBalNum==localRep.paxosHistory[paxosInstance].promiseBal.balNumber
				   &&sendBalPID>=localRep.paxosHistory[paxosInstance].promiseBal.PID){
	    			...
	    			if(localRep.paxosHistory[paxosInstance].acceptIPs.size()>2){
	    				...
	    				if(localRep.paxosInstance == paxosInstance){
                                                localRep.paxosInstance++;
                                        }
